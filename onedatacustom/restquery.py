...
class RestQueryClass:
    ...
    def get_attributefromfile(self, spaceAndRootAndfileName):
        url = "https://" + self.hostname +"/api/v3/oneprovider/attributes/" + spaceAndRootAndfileName
        response = requests.request("GET", url, headers=self.headers, verify=False)
        #print(url)
        #print(response.text)
        return (json.loads(response.text))

    def get_attributeextendedfromfile(self, spaceAndRootAndfileName):
        url = "https://" + self.hostname +"/api/v3/oneprovider/attributes/" + spaceAndRootAndfileName+"?extended=true"
    ...
    def retrieveMetadata(self, spaceAndRoot, scalefactor, nbr_of_file_per_directory):
       ...
       with open('./resultQuery/resultExtraction.csv', 'w', newline='') as csvfile:
            ...
            for j in range (1,scalefactor+1):
                pathdirectory1=spaceAndRoot+"/"+str(j)
...
if __name__ == '__main__':
   ...
   restquery.retrieveMetadata("/SPACE-SMALL/testUpdateMetaOnTheFlyB",2,500)
